...
def submit(request, course_id):
    ...
    for answer in answers:
        submission.choices.add(Choice.objects.filter(id = int(answer)).get())
...
def show_exam_result(request, course_id, submission_id):
    ...
    score = 0
    for choice in submit.choices.all().filter(correct=True).values_list('question_id'):
        score += Question.objects.filter(id=choice[0]).first().grade    
